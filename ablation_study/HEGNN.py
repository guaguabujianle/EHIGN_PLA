...
class DTIPredictor(nn.Module):
    def __init__(self, node_feat_size, edge_feat_size, hidden_feat_size, layer_num=3):
        ...
        self.inter_atompairs = AtomAtomAffinities(hidden_feat_size, edge_feat_size, hidden_feat_size)

        # bias correction
        self.bias_ligandpocket = BiasCorrectionLigandPocket(hidden_feat_size, edge_feat_size, hidden_feat_size)
        self.bias_pocketligand = BiasCorrectionPocketLigand(hidden_feat_size, edge_feat_size, hidden_feat_size)
    ...
    def forward(self, bg):
        ...
        bond_feats = {
            ('ligand', 'intra_l', 'ligand'): bond_feats[('ligand', 'intra_l', 'ligand')][:,-1].unsqueeze(-1) * 10,
            ('pocket', 'intra_p', 'pocket'): bond_feats[('pocket', 'intra_p', 'pocket')][:,-1].unsqueeze(-1) * 10,       
            ('ligand', 'inter_l2p', 'pocket'): bond_feats[('ligand', 'inter_l2p', 'pocket')][:,-1].unsqueeze(-1) * 10,    
            ('pocket', 'inter_p2l', 'ligand'): bond_feats[('pocket', 'inter_p2l', 'ligand')][:,-1].unsqueeze(-1) * 10,        
        }
